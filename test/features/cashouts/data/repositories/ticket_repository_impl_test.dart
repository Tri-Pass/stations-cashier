...
void main() {
  ...
  group('getDriverTickets', () {
    test('returns empty driver tickets entity', () async {
      when(() => dataSource.getDriverTickets(any()))
          .thenAnswer((_) async => {
    ...
    test('parses tickets correctly', () async {
      when(() => dataSource.getDriverTickets(any()))
          .thenAnswer((_) async => {
                ...
                'tickets': [
                  {
                    '_id': 'tk1',
                    'line': {'_id': 'l1', 'origin': 'Rabat', 'destination': 'Casa', 'price': 50.0},
                    'totalSeats': 2,
                    'paidMethod': 'cash',
                    'amount': 100.0,
                    'status': 'unpaid',
      ...
      final result = await repo.getDriverTickets(
          const GetDriverTicketsParams(driverId: 'd1'));
